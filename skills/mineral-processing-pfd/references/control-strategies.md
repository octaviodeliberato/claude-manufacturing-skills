...
Research §1 and §5: every supervisory product whose literature was read in full (Metso Grinding
Optimizer, Andritz BrainWave, Honeywell Profit Controller, Mintek MillStar, Metso IC70C) is a separate
application whose outputs are *setpoints* of existing regulatory or enhanced-regulatory controllers;
Andritz documents the OPC link to the DCS and the watchdog fallback to DCS control [Gough p.5], and the
same shape is claimed for Sandvik ASRi and FLSmidth ProcessExpert / LoadIQ *(search snippets only,
research §5–6)*. The drawing says exactly that and no more (ADR 0004):
