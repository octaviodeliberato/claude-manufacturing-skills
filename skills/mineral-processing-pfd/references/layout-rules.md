...
Layout geometry and the QA checklist. Forked from `pfd-generator/references/layout-rules.md`
(same reasoning as `docs/adr/0001-mineral-processing-pfd-forks-primitives.md`: skills are
distributed one at a time, so a cross-skill reference isn't viable). The geometry and QA
discipline are domain-agnostic — only a few equipment-specific asides below have been
reworded from "vessel" to "equipment" for this skill's symbol set.
...
| Equipment | 300–640 | Main equipment run (crushing → grinding → classification → separation → dewatering) |
| Lower process | 650–780 | Bottom discharges, underflows, tailings |
...
- [ ] Text inside equipment does not collide with an internal detail (mill charge circles, deck mesh lines, agitator shaft, impeller)
