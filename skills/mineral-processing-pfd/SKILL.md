...
- If the user asks for a **control strategy** (control philosophy, loops, instrumentation, automation) on a crushing or grinding circuit: resolve **one control tier per circuit present** (crushing, grinding) — basic, intermediate or advanced, see `references/control-strategies.md`. A single tier named without a circuit is the flowsheet-wide default and applies to every circuit; a tier named for one circuit ("intermediate on grinding") applies to that circuit only, and any circuit left unnamed gets **basic**. Don't ask — assume, and state each assumed tier in the delivery. **Advanced is never drawn unasked**: only when the user names it (advanced, expert system, MPC, optimiser, APC, supervisory control), and only on the circuit they name it for. Flotation/thickening control strategies aren't in the reference yet — say so rather than improvising.
...
- **Generic** (forked from `pfd-generator`, unchanged behavior apart from `legend`'s `density` flag): `pipe`, `sig`, `lead`, `line_jump`, `bubble`, `cvalve`, `manual_valve`, `vessel`, `agitator`, `equip_tag`, `legend`, `title`, `notes`, `design_basis`, `revision`; plus `motor` (ISA drive symbol, the final element for speed loops), `supervisory_block` (the advanced-tier block) and `softlink` (ISA-5.1 software/data link, the only signal style that may leave it) — all three new here, not in `pfd-generator`.
...
**Control strategy.** When a strategy was requested, read `references/control-strategies.md` and draw each circuit's resolved tier — nothing more, nothing less. Drive-speed final elements (feeder, mill, pump VSDs) terminate on `motor()`, never on a `cvalve`; it returns the point the controller's signal lands on. Water loops terminate on `cvalve`. Crusher-setting (CSS) loops are a `ZIC` whose output `sig` ends on the crusher body — one controller with setting mode and load mode as alternative modes, never two loops. Monitors (mill power, PSM, belt scale, a leftover pressure) stop at an indicator bubble. Sump level and cyclone feed density cannot both sit on the dilution-water valve — pick the pairing the reference describes and state it. Any `DT`/`DIC` needs `legend(..., density=True)`. Stamp the sheet with the per-circuit tier note via `notes()`: first bullet of a `CONTROL STRATEGY` block, every circuit listed as `<Circuit>: <tier>` separated by ` | `, `(assumed)` after any tier the user didn't name — e.g. `"Crushing: basic (assumed) | Grinding: intermediate"`. `examples/build_grinding_circuit_control.py` is the pattern to copy (crushing at basic, grinding at intermediate).

**Cascade, ratio and override (intermediate tier).** Tiers are cumulative (ADR 0004): draw the basic loops first and add the intermediate ones on top — never delete or re-route a basic loop. A master's output lands on the slave bubble's **setpoint port** (a port the measurement does not use), lettered `SP` beside the arrowhead, so a cascade reads as a cascade; ratio, override and feedforward are `Y` bubbles (`FFY`, `JY`/`PY`/`LY`, `WY`) between master and slave, never implied by a line. This follows the `pfd-generator` cascade convention because the ISA-5.1 clause could not be sourced — the reference's "Drawing a cascade, ratio or override" section has the port, labelling and crossing rules.

**Supervisory block and software link (advanced tier).** Draw advanced as **one `supervisory_block` per circuit** — a titled rounded rectangle ("SAG MILL OPTIMISER (MPC)", "CRUSHER OPTIMISER") — connected by `softlink()` from a port it returns to the **setpoint port** of each tier-2 master it drives, lettered `SP` at the arrowhead like any cascade leg. Never a computer-function hexagon per loop, never a dashed `sig` from the block (it would read as one more cascade), never a link onto a slave whose setpoint a master already owns, and **never unasked**. The block has no loop tag. Register the style in the legend with `("Software link (setpoint)", SIG, "soft")` — and because that fifth entry pushes the `D = density` declaration off a 1520-wide sheet, keep legend labels short or split the legend into two `legend()` rows. The reference's "Drawing the advanced tier" section and each circuit's advanced row say which controllers the block lands on; the shipped example stays at crushing basic · grinding intermediate, so the advanced layout is yours to plan (block in its own band above the circuit's controllers, one corridor y per link so links never cross each other).
...
Before shipping, also run the smoke tests, which catch structural defects (missing arrowheads on `sig` and `softlink`, raster images, malformed SVG) before you even look at a render:
...
- When a control strategy was drawn: the **control narrative** table (columns `Loop tag | Tier | Measured variable | Manipulated variable / final element | Objective | Notes`, one row per loop, monitors marked as such, master/slave named in Notes for every cascade, ratio and override row, and any conditional intermediate row omitted — feedforward with no measured source, ball-addition ratio with no ball feeder — listed with the reason) one row per software link when a supervisory block was drawn (`Loop tag` = the block title, `Tier` = advanced, `Manipulated variable` = the setpoint written, and the operating philosophy — hold PSE / maximise throughput or hold throughput / push PSE — for a grinding block), vendor performance figures quoted as vendor claims, and the per-circuit tier line in the same format as the drawing's note ("Crushing: basic (assumed) | Grinding: basic (assumed) — no tier was named"), the CSS mode drawn, plus the level/density pairing chosen
