...
**Supervisory block and software link (advanced tier).** Draw advanced as **one `supervisory_block` per circuit** — a titled rounded rectangle ("SAG MILL OPTIMISER (MPC)") — connected by `softlink()` from a port it returns to the **setpoint port** of each tier-2 master it drives, lettered `SP` at the arrowhead. Never a computer-function hexagon per loop, never a dashed `sig` from the block, never a link onto a slave whose setpoint a master already owns, and **never unasked**. Which controllers the block lands on, the legend entry, and the routing pattern are in the reference's "Drawing the advanced tier" section and each circuit's advanced row; the shipped example stays at crushing basic · grinding intermediate, so the advanced layout is yours to plan.
