...
- When a control strategy was drawn: the **control narrative** table (columns `Loop tag | Tier | Measured variable | Manipulated variable / final element | Objective | Notes`, one row per loop, monitors marked as such) and the per-circuit tier line in the same format as the drawing's note ("Crushing: basic (assumed) | Grinding: basic (assumed) — no tier was named"), the CSS mode drawn, plus the level/density pairing chosen
